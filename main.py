...
with st.echo(code_location='below'):
        ...
        ## Веб-скреппинг: Использовался и BeautifulSoap, и Selenium.
        ## Работа с REST API (XML/JSON). Использовала для получения геоданных о ресторанах Москвы, входящих в реестр Мишлен.
        ## Визуализация данных: Построение графиков, некоторые из которых имеют нетривиальный код. Такж две разные карты.
        ## Работа с графами (библиотека networkx): Использавалась.
        ## Математические возможности Python: Numpy использовался для нахождения корреляции.
        ## Streamlit: Работа сделана в Streamlitе.
        ## SQL: Использовался.
        ## Регулярные выражения: Использовались (где использовались, есть описание).
        ## Работа с геоданными: Применялась, с помощью библиотеки folium.
        ## Машинное обучение: Использовалось (Пицца).
        ## Дополнительные технологии (библиотеки, не обсуждавшиеся в ходе курса): Использовала новую библиотеку wikipedia, изучила её документацию, также изучила документацию folium и применила возможности, не обсуждавшиеся в ходе курса. Библиотеки seaborn и scipy.
        ## Объём (осмысленных строк кода, не считая import): Больше 120.
        ## Целостность проекта и Общее впечатление это субъективные параметры, но надеюсь, что моя работа вам понравилась, что она была полезной, инфоомативной, а может быть даже немного весёлой.
        ...
        st.markdown("P.S.3 (Если вместо карты вы видите жёлтую рамку, немного подождите, не нервничайте, это нормально, заварите пока чаек с мятой, он успокаивает)")
        ...
        st.image("https://avatars.mds.yandex.net/i?id=6f0f9384b53314d8e6d72b0a64ec2148-5664046-images-thumbs&n=13", width = 250)
        st.header("Пиццаааа")
        ...
        st.pyplot(figure)
        st.markdown("Вершины 0-4 обозначают категории цен, а вершины 5-7 обозначают добавки, заметим, что на графе каждая вершина из категории добавок связяна с каждой вершиной из категории цен.")
